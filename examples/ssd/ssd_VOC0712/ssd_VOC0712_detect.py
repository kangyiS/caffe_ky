...
save_img = False # save images or video
gpu_id = 0
...
class CaffeDetection:
    ...
    def detect(self, image_file, conf_thresh=0.5, topn=500):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Detect an image, a group of images or a video")
    ...
    if os.path.isfile(source_file) and file_type is "image": # detect one image
        ...
        result = detection.detect(source_file,conf_thresh)
        if save_img:
        ...
        if cap.isOpened():
            ...
            while success:
                ...
                if success:
                    ...
                    result = detection.detect(frameRGB,conf_thresh)
        ...
        for root, dirs, files in os.walk(source_file):
            for file in files:
                if os.path.splitext(file)[1] == '.jpg':
                    ...
                    result = detection.detect(img_file,conf_thresh)
                    if count % 1000 == 0:

...
//这个函数在判断是否要更新solver type，老版本和新版本的type书写不一样
bool SolverNeedsTypeUpgrade(const SolverParameter& solver_param) {
  if (solver_param.has_solver_type()) {//has_solver_type()里面保存的是老版本的solver type，如果有，就需要更新一下
    return true;
  }
  return false;
}
//这里更新了一下老版本和新版本的不同，两个版本下solver type是不同的，老版本写的比较长
//比如将SolverParameter_SolverType_SGD更新为了SGD
...
//如果需要更新solver type的话，就更新一下，因为老版本和新版本的solver type书写不一样，老版本的比较麻烦
bool UpgradeSolverAsNeeded(const string& param_file, SolverParameter* param) {
  ...
  if (SolverNeedsTypeUpgrade(*param)) {//如果需要更新solver type（因为两个版本的solver type书写不一样）
    LOG(INFO) << "Attempting to upgrade input file specified using deprecated "
              << "'solver_type' field (enum)': " << param_file;
    if (!UpgradeSolverType(param)) {//更新了solver type，老版本的书写麻烦，新版本的简单
      success = false;
      LOG(ERROR) << "Warning: had one or more problems upgrading "
...
void ReadSolverParamsFromTextFileOrDie(const string& param_file,
  ...
  CHECK(ReadProtoFromTextFile(param_file, param))//param保存了param_file文件里的参数
      << "Failed to parse SolverParameter file: " << param_file;//如果解析失败，是要终止程序的   FATAL
	  //这个函数调用的param_file文件，只是在函数里面用作info输出用，更新的数据都是param里面的
  UpgradeSolverAsNeeded(param_file, param);//检查一下看看param里面的solver type是否需要更新
}

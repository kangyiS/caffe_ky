...
//如果a大于等于0，并且小于b，那么返回true。输入的参数b一定是正数！！！！！！
inline bool is_a_ge_zero_and_a_lt_b(int a, int b) {
  return static_cast<unsigned>(a) < static_cast<unsigned>(b);
}
/*im2col_cpu将c个通道的卷积层输入图像转化为c个通道的矩阵，矩阵的行值为卷积核高*卷积核宽， 
也就是说，矩阵的单列表征了卷积核操作一次处理的小窗口图像信息；而矩阵的列值为一共要处理小
窗口的个数。
这一个函数不是在进行真正的卷积运算，而是将输入的图像矩阵按照一种方式转换为了另一个矩阵， 
方便使用math_function()里面的函数进行矩阵运算。
